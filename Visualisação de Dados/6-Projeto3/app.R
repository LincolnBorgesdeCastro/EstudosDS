...
server <- function(input, output) { 
  ...
  output$dygraph <- renderDygraph({
    ...
    ?dySeries
    dygraph(ga2Sub) %>% 
      dySeries("ses", label = "Sessões",axis="y",stepPlot=TRUE,fillGraph=TRUE, color="red") %>% 
      dySeries("con",fillGraph=TRUE, label = "Transações",axis="y", color="white") %>% 
      dySeries("cr", label = "Taxa de Conversão",axis="y2",strokePattern = "dashed", color="black") %>% 
